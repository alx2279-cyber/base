...
// Generate random window class name
static std::string GenerateRandomClassName() {
    const char* prefixes[] = {
        "Windows", "DirectUI", "Shell", "Mso", "Afx", "Static",
        "Button", "ComboBox", "Edit", "ListBox", "ScrollBar"
    };
    
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> prefixDist(0, sizeof(prefixes) / sizeof(prefixes[0]) - 1);
...
namespace RenderCore
{
    bool D3DDevice::CreateDeviceD3D(HWND hWnd)
    {
        ...
        sd.BufferDesc.RefreshRate.Numerator = 0; // Unlock refresh rate
...
namespace RenderCore
{
    ...
    {
        auto targetFrameTime = std::chrono::microseconds(5555);

        while (!EndFlag)
        {
            auto frameStart = std::chrono::steady_clock::now();
            
            targetFrameTime = std::chrono::microseconds(1000000 / TargetFPS);
            ...
            g_Device.g_pSwapChain->Present(0, 0); // Present WITHOUT VSync

            // Frame Limiter for 180 FPS
            auto frameEnd = std::chrono::steady_clock::now();
            auto elapsedTime = std::chrono::duration_cast<std::chrono::microseconds>(frameEnd - frameStart);

            if (elapsedTime < targetFrameTime) {
                if (targetFrameTime - elapsedTime > std::chrono::milliseconds(2)) {
                     std::this_thread::sleep_for(std::chrono::milliseconds(1));
                 }
                 while (std::chrono::duration_cast<std::chrono::microseconds>(std::chrono::steady_clock::now() - frameStart) < targetFrameTime) {
    ...
    bool OSImGui_External::CreateMyWindow()
    {
        ...
        {
            Window.hWnd = CreateWindowExW(WS_EX_TOPMOST | WS_EX_TRANSPARENT | WS_EX_LAYERED, Window.wClassName.c_str(), Window.wName.c_str(), WS_POPUP, (int)Window.Pos.x, (int)Window.Pos.y, (int)Window.Size.x, (int)Window.Size.y, NULL, NULL, wc.hInstance, NULL);
            SetLayeredWindowAttributes(Window.hWnd, 0, 255, LWA_ALPHA);
        }
